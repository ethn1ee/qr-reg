...
export default function QrScanner() {
  ...
  // Keep the ref updated with the latest score
  ...
  useEffect(() => {
    ...
    const onScanSuccess = (decodedText: string) => {
      ...
      if (!isNaN(Number(decodedText))) {
        ...
        // Use the score from the ref, which is always up-to-date

        handleUpdateScore(decodedText, scoreRef.current);
  ...
  const handleUpdateScore = async (scannedRow: string, scoreToUpdate: number) => {
    ...
    try {
      ...
      const response = await fetch('/api/update-score', {
        ...
        body: JSON.stringify({ row: scannedRow, score: scoreToUpdate }),
